...
`from: harness` 表示借用宿主 CLI 登录态；显式字段覆盖借用值。token 过期或缺失 → 显式 missing，不抛异常。

加载顺序：env 字段覆盖 > `E2E_LIVE_CONFIG_PATH` 显式文件 > repo 内 `.env.e2e.yaml` > `~/.config/x-agent-suite/.env.e2e.yaml`。缺文件/缺 carrier 时 live 用例按「未配置」skip，不判红。
