...
聚合模块只承担分发接口，不复制领域逻辑。将来确有消费者只需单包安装时，再把对应内部模块提升为独立分发模块，不提前扩大发布面。按需安装形态（pip extras 等价物、触发条件与切片纪律）的完整评估见 [`research/on-demand-install-artifact-matrix.md`](../research/on-demand-install-artifact-matrix.md)。
