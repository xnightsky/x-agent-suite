...
> 每个能力件都是**可替换槽位**：账本「实现归属」列给的是默认实现，消费者可整段替换而不改上下游契约，见根 README「流水线阶段」槽位表。
...
| 能力件                   | 概念来源                                  | 裁决                              | 归属                           | 状态                                       |
| ------------------------ | ----------------------------------------- | --------------------------------- | ------------------------------ | ------------------------------------------ |
| repeat 稳定率 / 成本方差 | promptfoo repeat / Inspect epochs+reducer | 内核                              | `runner/repeat` + 报告稳定率列 | ✅ P5（成本方差待 token 层数据）           |
| 派生指标（表达式）       | promptfoo derivedMetrics                  | **子包依赖 mathjs**，不自造解析器 | 未排期                         | ❌ 缓（P5 统计用纯算术，表达式合成未出现） |
